    ...
    if (isLoading) {
        return (
            <div className="grid gap-4">
                {Array(3).fill(0).map((_, i) => <ProcedimentoCardSkeleton key={i} />)}
            </div>
        );
    }
        ...
        return (
            <Card className="text-center py-12 bg-white shadow-sm border-slate-200">
                <CardContent>
                    ...
                    <h3 className="text-xl font-semibold text-slate-700 mb-2">Nenhum procedimento encontrado</h3>
                    <p className="text-slate-500">Comece criando seu primeiro procedimento padrão.</p>
                </CardContent>
            </Card>
        );
    }
