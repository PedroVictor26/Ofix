...
export default function ServiceProcedures({ service, procedimentos, onUpdate }) {
    const [searchTerm, setSearchTerm] = useState("");
    const [isApplying, setIsApplying] = useState(false);

    const filteredProcedimentos = procedimentos.filter(proc =>
        proc.nome_procedimento.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    const handleApplyProcedure = async (procedimento) => {
        ...
        try {
            ...
            const updatedData = {
                descricao_problema: service.descricao_problema || procedimento.descricao_padrao,
            ...
            <div className="grid gap-4">
                {filteredProcedimentos.map((procedimento) => (
                    <Card key={procedimento.id} className="hover:shadow-md transition-shadow">
                        ...
                        <CardContent>
                            <p className="text-slate-600 mb-4">{procedimento.descricao_padrao}</p>

                            {procedimento.checklist_padrao && procedimento.checklist_padrao.length > 0 && (
                                <div>
                                    ...
                                    <ul className="text-sm text-slate-600 space-y-1">
                                        {procedimento.checklist_padrao.slice(0, 3).map((item, index) => (
                                        ...
                                        {procedimento.checklist_padrao.length > 3 && (
                                            <li className="text-slate-400 italic">
                                                +{procedimento.checklist_padrao.length - 3} itens adicionais
                                            </li>
                                        )}
                                    </ul>
                                </div>
