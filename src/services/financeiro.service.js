...
export const getFaturamentoHoje = async () => {
  try {
    const response = await apiClient.get('/financeiro/faturamento-hoje');
    return response.data;
  } catch (error) {
    console.error("Erro ao buscar faturamento de hoje:", error.response?.data?.error || error.message);
    throw error.response?.data || { message: error.message || "Erro desconhecido ao buscar faturamento de hoje." };
  }
};